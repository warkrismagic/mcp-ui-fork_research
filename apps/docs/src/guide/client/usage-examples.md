...
// Assuming InteractiveHtmlResourceBlock structure comes from @mcp-ui/shared or a compatible type
import type { InteractiveHtmlResourceBlock } from '@mcp-ui/shared'; 

// Mock function to simulate receiving an MCP resource block
const fetchMcpResource = async (id: string): Promise<InteractiveHtmlResourceBlock> => {
...
const App: React.FC = () => {
  const [resourceBlock, setResourceBlock] = useState<InteractiveHtmlResourceBlock | null>(null);
